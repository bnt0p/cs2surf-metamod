...
static_assert(SURF_ARRAYSIZE(enforcedServerCVars) == ENFORCEDCVAR_COUNT, "Array enforcedServerCVars length is not the same as ENFORCEDCVAR_COUNT!");

// clang-format on

void SurfGlobalService::EnforceConVars()
{
...
void SurfGlobalService::RestoreConVars()
{
	for (u32 i = 0; i < ENFORCEDCVAR_COUNT; i++)
	{
		ConVarData *data = enforcedServerCVars[i]->GetConVarData();
