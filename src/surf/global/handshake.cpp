...
bool Surf::API::handshake::Hello::ToJson(Json &json) const
{
	// clang-format off
...
bool Surf::API::handshake::Hello::PlayerInfo::ToJson(Json &json) const
{
	return json.Set("id", this->id) && json.Set("name", this->name);
}

bool Surf::API::handshake::HelloAck::FromJson(const Json &json)
{
...
bool Surf::API::handshake::HelloAck::ModeInfo::FromJson(const Json &json)
{
	std::string mode;
	...
	if (!Surf::API::DecodeModeString(mode, this->mode))
	{
		return false;
	}
...
bool Surf::API::handshake::HelloAck::StyleInfo::FromJson(const Json &json)
{
	std::string style;
	...
	if (!Surf::API::DecodeStyleString(style, this->style))
	{
		return false;
	}
