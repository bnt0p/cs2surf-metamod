...
BaseRequest::BaseRequest(u64 uid, SurfPlayer *player)
	: uid(uid), userID(player->GetClient()->GetUserID()), timestamp(g_pSurfUtils->GetServerGlobals()->realtime)
{
}
...
void BaseRequest::Init(u64 features, const CCommand *args, bool queryLocal, bool queryGlobal)
{
	...
	KeyValues3 params;
	if (!utils::ParseArgsToKV3(args->ArgS(), params, const_cast<const char **>(paramKeys), SURF_ARRAYSIZE(paramKeys)))
	{
		this->Invalidate("");
		this->PrintInstructions();
		return;
	}
	this->localStatus = (queryLocal && SurfDatabaseService::IsReady()) ? ResponseStatus::ENABLED : ResponseStatus::DISABLED;

	this->globalStatus = (queryGlobal && SurfGlobalService::IsAvailable() ? ResponseStatus::ENABLED : ResponseStatus::DISABLED);
	...
	if (this->HasFeature(RequestFeature::Map))
	{
		...
		if (this->mapName.IsEmpty())
		{
			...
			CUtlString currentMap = g_pSurfUtils->GetCurrentMapName(&gotCurrentMap);
...
void BaseRequest::CheckRequests()
{
	...
	instances.erase(std::remove_if(instances.begin(), instances.end(),
		[](BaseRequest *instance)
		{
			...
			else if (instance->timestamp + instance->timeout < g_pSurfUtils->GetServerGlobals()->realtime)
			{
...
void BaseRequest::SetupCourse(CUtlString courseName)
{
	...
	if (this->mapName == currentMap)
	{
		...
		const SurfCourseDescriptor *course = callingPlayer->timerService->GetCourse();
		if (!course)
		{
			// No course? Take the map's first course.
			course = Surf::course::GetFirstCourse();
			if (!course)
			{
	...
	else if (this->localStatus == ResponseStatus::ENABLED)
	{
		...
		SurfDatabaseService::FindFirstCourseByMapName(mapName, onQuerySuccess, onQueryFailure);
	}
	else
	{
		courseName = "1";
...
void BaseRequest::SetupPlayer(CUtlString playerName)
{
	...
	SurfPlayer *callingPlayer = g_pSurfPlayerManager->ToPlayer(this->userID);
	if (!callingPlayer)
	{
		this->Invalidate();
	}
	...
	if (this->localStatus == ResponseStatus::ENABLED)
	{
		...
		SurfDatabaseService::FindPlayerByAlias(playerName, onQuerySuccess, onQueryFailure);
	}
...
void BaseRequest::SetupMode(CUtlString modeName)
{
	...
	if (modeName.IsEmpty())
	{
		SurfModeManager::ModePluginInfo modeInfo = Surf::mode::GetModeInfo(callingPlayer->modeService);
	...
	else
	{
		...
		SurfModeManager::ModePluginInfo modeInfo = Surf::mode::GetModeInfo(modeName);
	...
	if (!Surf::API::DecodeModeString(this->modeName.Get(), this->apiMode))
	{
...
void BaseRequest::SetupStyles(CUtlString styleNames)
{
	...
	if (styleNames.IsEmpty())
	{
		...
		{
			SurfStyleManager::StylePluginInfo info = Surf::style::GetStyleInfo(callingPlayer->styleServices[i]);
	...
	else
	{
		...
		{
			SurfStyleManager::StylePluginInfo info = Surf::style::GetStyleInfo(stylesSplit[i]);
			if (info.databaseID < 0)
			{
				this->localStatus = ResponseStatus::DISABLED;
			}
